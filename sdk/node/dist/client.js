...
export class AgentMeter {
    ...
    constructor(opts = {}) {
        ...
        this.endpoint = (opts.endpoint ?? process.env.AGENT_METER_ENDPOINT ?? DEFAULT_ENDPOINT).replace(/\/$/, "");
    ...
    finish(span, ok = true, error) {
        span.endedAtNs = process.hrtime.bigint();
        span.durationMs = Number((span.endedAtNs - span.startedAtNs) / 1000000n);
        span.ok = ok;
        if (error)
            span.error = error;
    }
    ...
    buildOtlpPayload(spans) {
        const otlpSpans = spans.map((s) => {
            ...
            if (s.model)
                attrs.push({ key: "gen_ai.request.model", value: { stringValue: s.model } });
            if (s.mcpServer)
                attrs.push({ key: "mcp.server", value: { stringValue: s.mcpServer } });
            if (s.ide)
                attrs.push({ key: "ide", value: { stringValue: s.ide } });
            if (s.agent)
                attrs.push({ key: "agent", value: { stringValue: s.agent } });
            if (s.conversationId)
                attrs.push({ key: "session.id", value: { stringValue: s.conversationId } });
            if (s.taskId)
                attrs.push({ key: "task.id", value: { stringValue: s.taskId } });
            ...
            if (s.usdCost != null)
                attrs.push({ key: "cost.usd", value: { doubleValue: s.usdCost } });
            if (s.error)
                attrs.push({ key: "error.message", value: { stringValue: s.error } });
