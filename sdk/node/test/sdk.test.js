...
describe("AgentMeter", () => {
  ...
  async function closeWithoutFlush(am) {
    am["buffer"] = [];
    await am.shutdown();
  }

  it("track creates a span with correct fields", async () => {
  ...
  it("finish sets duration and timing", async () => {
  ...
  it("finish with error sets ok=false", async () => {
    const am = new AgentMeter({ apiKey: "test", endpoint: "http://localhost:3000" });
  ...
  it("buildOtlpPayload produces valid structure", async () => {
    const am = new AgentMeter({ apiKey: "test", endpoint: "http://localhost:3000" });
