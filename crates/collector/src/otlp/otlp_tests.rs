    ...
    fn extract_user_request_tag_skips_conversation_summary() {
        let content = "<conversation-summary>see <userRequest>docs</userRequest></conversation-summary>";
        assert_eq!(extract_user_request_tag(content), None);
    }

    #[test]
    fn extract_user_request_tag_outside_summary_ok() {
        let content = "<conversation-summary>old</conversation-summary><userRequest>new prompt</userRequest>";
        assert_eq!(
            extract_user_request_tag(content).as_deref(),
            Some("new prompt")
        );
    ...
    fn extract_user_request_tag_empty_or_noise() {
        assert_eq!(extract_user_request_tag("<userRequest>  </userRequest>"), None);
    ...
    fn parse_first_human_text_plain_string_content() {
        let raw = r#"[{"role":"assistant","content":"hi"},{"role":"user","content":"  do the thing  "}]"#;
        assert_eq!(
            parse_first_human_text(raw).as_deref(),
            Some("do the thing")
        );
    ...
    fn is_generic_http_method_table() {
        for name in [
            "GET", "post", "PUT", "DELETE", "PATCH", "HEAD", "OPTIONS", "HTTP GET", "HTTP POST",
    ...
    fn is_copilot_http_span_rejects_unrelated_url() {
        let span = span_with_attrs(
            "GET",
            vec![attr_str("http.url", "https://example.com/api")],
        );
    ...
    async fn test_buffer(
        ...
        let buf = crate::services::ingest_buffer::IngestBuffer::spawn(db.clone(), capacity, cancel.clone());
        (db, buf, cancel)
    }
    ...
    async fn handle_json_invalid_and_empty_branches() {
        ...
        let bad_proto = handle_trace_request(b"\x00\x01\x02", Some("application/x-protobuf"), None, None, None);
    ...
    fn parse_first_human_text_and_noise_branches() {
        ...
        assert_eq!(parse_first_human_text(blocks).as_deref(), Some("block user"));

        let tool_result = r#"[{"role":"user","content":[{"type":"tool_result","text":"x"}]}]"#;
        assert_eq!(parse_first_human_text(tool_result), None);

        assert!(is_noise_prompt("You are a helpful assistant with a long system prompt"));
    ...
    fn extract_user_request_skips_conversation_summary() {
        ...
        let inside = "<conversation-summary>mentions <userRequest>docs</userRequest></conversation-summary>";
    ...
    async fn handle_json_rich_chat_models() {
        ...
        let spans = serde_json::json!([tool_span_json("execute_tool a"), tool_span_json("execute_tool b"), tool_span_json("execute_tool c")]);
        let body = json_otlp_payload(spans, "cursor");
    ...
    async fn json_map_error_paths_and_unknown_spans() {
        ...
        let _ = handle_trace_request(&body, Some("application/json"), Some("1.1.1.1"), Some("ua"), Some(&buf));
    }
    ...
    fn parse_first_human_text_edge_branches() {
        ...
        assert_eq!(parse_first_human_text(only_ctx).as_deref(), Some("fallback"));
        // content XML with userRequest (556)
        let ctag = r#"[{"role":"user","content":"<wrapper><userRequest>c tag</userRequest></wrapper>"}]"#;
        ...
        assert_eq!(parse_first_human_text(empty_block).as_deref(), Some("block ok"));
    ...
    fn extract_clean_prompt_noise_and_indexed() {
        ...
        let indexed = vec![
            ...
            attr_str("gen_ai.prompt.0.content", "sys"),
            attr_str("gen_ai.prompt.1.role", "user"),
            attr_str("gen_ai.prompt.1.content", "<userRequest>indexed req</userRequest>"),
        ];
        assert_eq!(
        ...
        let indexed_noise = vec![
            ...
            attr_str("gen_ai.prompt.0.content", "Please write a brief title for this chat"),
        ];
    ...
    fn extract_response_text_all_formats() {
        ...
        assert_eq!(
            extract_response_text(&blocks).as_deref(),
            Some("block out")
        );
    ...
    async fn proto_map_errors_and_http_edges() {
        ...
        let spans = vec![
            ...
            full_proto_span(
                "chat c",
                vec![kv_str("gen_ai.response.model", "claude-3")],
            ),
            full_proto_span("chat g", vec![kv_str("gen_ai.response.model", "gpt-4")]),
            full_proto_span("chat o", vec![kv_str("gen_ai.response.model", "o1-preview")]),
            full_proto_span(
                "chat gm",
    ...
    fn coverage_remaining_otlp_edges() {
        ...
        let empty_blocks = r#"[{"role":"user","content":[{"type":"text","text":""},{"type":"image"}]}]"#;
    ...
    fn extract_response_empty_parts_then_none() {
        ...
        let obj = vec![attr_str("gen_ai.output.messages", r#"{"role":"assistant"}"#)];
    ...
    fn is_copilot_http_attrs_without_match_returns_false() {
        ...
        let span = span_with_attrs(
            "GET",
            vec![attr_str("server.address", "api.example.com")],
        );
        assert!(!is_copilot_http_span(&span, "GET"));
        let span = Span {
            name: "GET".into(),
